	...
	render(){
		...
		return(
			 ...
			 <li className="task">
			  ...
			  { (task.edit) ? <EditTaskForm task={task} index={index} setCurrentTaskEditable={this.props.setCurrentTaskEditable} updateTask={this.props.updateTask}/> : <div><button onClick={() => this.props.markasDone(index)} className="markcomplete">{markcompleteText}</button><button onClick={() => this.props.setCurrentTaskEditable(index)} className="editTask">Edit Task</button><button onClick={() => this.props.removefromTask(index)} className="removeTask">RemoveTask</button></div>		 }
			 </li>
			)
	}
